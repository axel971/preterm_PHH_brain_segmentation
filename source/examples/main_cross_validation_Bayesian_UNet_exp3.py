...
def main():
    ...
    for iFold in range(nFold):
    	...
    	if(_ONE_GPU_):
    		model = bayesian_Unet3d(patch_size + [1], len(labels), 0.4)
    	else:
    		single_model = bayesian_Unet3d(patch_size + [1], len(labels), 0.4)
    	...
    	model.save('/home/axel/dev/neonatal_brain_segmentation/data/output/bayesian_UNet/dropout_encoder_decoder/p_04/models/model_' + str(iFold) + '.h5')
    	...
    	for i in range(len(test_id)):
    		...
    		np.save('/home/axel/dev/neonatal_brain_segmentation/data/output/bayesian_UNet/dropout_encoder_decoder/p_04/MCsamples/allVOI_cross_entropy_loss/' + subject_names[test_id[i]] + '_MCsamples.npy', MCsamples)
    		save_image(prediction, affines[test_id[i]], '/home/axel/dev/neonatal_brain_segmentation/data/output/bayesian_UNet/dropout_encoder_decoder/p_04/prediction/allVOI_cross_entropy_loss/' + subject_names[test_id[i]] + '_predicted_segmentation.nii')
