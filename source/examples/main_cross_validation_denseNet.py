...
def main():
    ...
    for iFold in range(nFold):
    	...
    	if(_ONE_GPU_):
    		model = denseNet(patch_size+[1], len(labels))
    	else:
    		single_model = denseNet(patch_size+[1], len(labels))
    	...
    	print('..step_by_epoch:' + str(training_generator.step_by_epoch), flush=True)
    	print('..self.current_epoch:' + str(training_generator.current_epoch), flush=True)
    	...
    	print('taining time:' + str(end_training_time - start_training_time))
    	
    	model.save('/home/axel/dev/neonatal_brain_segmentation/data/output/denseNet/exp2/models/model_' + str(iFold) + '.h5')
    	...
    	print('execution time:' + str((end_execution_time - start_execution_time)/len(test_id)))
    	
    	for i in range(len(prediction)):
    		dice.append(metrics.dice_multi_array(Y[test_id[i]], prediction[i], labels))
    		save_image(prediction[i], affines[test_id[i]], '/home/axel/dev/neonatal_brain_segmentation/data/output/denseNet/exp2/prediction/allVOI_crossEntropy/' + subject_names[test_id[i]] + '_predicted_segmentation.nii')
