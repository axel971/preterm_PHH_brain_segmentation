...
def main():
	...
	MCsamples_dir = '/home/axel/dev/neonatal_brain_segmentation/data/output/bayesian_UNet/dropout_encoder_decoder/p_01_exp2/MCsamples/allVOI_cross_entropy_loss'
	image_dir = '/home/axel/dev/neonatal_brain_segmentation/data/images/case/preprocessing' 
	volume_uncertainty_dir = '/home/axel/dev/neonatal_brain_segmentation/data/output/bayesian_UNet/dropout_encoder_decoder/p_01_exp2/volume_uncertainty/volume_uncertainty.csv'
...
# 	for iSubject in range(n_subject):
	for iSubject in range(0,1):
		print("Subject: " + subject_names[iSubject])
		MCsamples = np.load(MCsamples_dir + '/' + subject_names[iSubject] + '_MCsamples.npy')
		for iMCsamples in range(len(MCsamples)):
			print(MCsamples[iMCsamples,128,64,128])
		
		print("Subject proba: " + subject_names[iSubject])
		MCsamples = MCsamples / np.sum(MCsamples, axis = -1, keepdims = True)
		for iMCsamples in range(len(MCsamples)):
			print(MCsamples[iMCsamples,128,64,128])
		
# 		print("Subject proba sum: " + subject_names[iSubject])
# 		sum = np.sum(MCsamples, axis = -1)
# 		for iMCsamples in range(len(MCsamples)):
# 			print(sum[iMCsamples,128,64,128])
