...
for(iterInputImg.GoToBegin(), iterOutputImg.GoToBegin(); !iterInputImg.IsAtEnd(), !iterOutputImg.IsAtEnd(); ++iterInputImg, ++iterOutputImg)
{
		...
		if(iterInputImg.Get() == 5 ) //Lateral Ventricles
			iterOutputImg.Set(1);
			
		else if(iterInputImg.Get() == 1 ) //External CSF
			iterOutputImg.Set(2);
			...
			iterOutputImg.Set(4); //Cortical gray matter
		
		else if(iterInputImg.Get() == 6 )
			iterOutputImg.Set(5); //Cerebellum
						
		else if(iterInputImg.Get() ==  8 )
			iterOutputImg.Set(6); //Brainstem			
