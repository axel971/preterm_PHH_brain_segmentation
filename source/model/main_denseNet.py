import os
import numpy as np
from model import tiramisu, denseNet
...
def main():
    ...
    for n in range(n_subject):
        # X[n] = resize(load_image_correct_oritation('rawdata/'+str(subject_list[n])+'x.nii'),image_size)
        # Y[n] = redefine_label(resize(load_image_correct_oritation('rawdata/'+str(subject_list[n])+'y.nii'),image_size, islabel=True))
...
#     printgpu('..n_epoch_all_data:' + str(training_generator.n_epoch_all_data))
    ...
    validation_generator = Generator(X[ID:, ...], Y[ID:, ...],
                                     batch_size=8, patch_size=patch_size,
                                     labels=labels, argmentation=False)
    ...
    # check_pointer = ModelCheckpoint(filepath='auto_save_weight.h5',verbose=1,
    #                             save_best_only=True, save_weights_only=True)
    tensor_board_callback = TensorBoard(log_dir='./logDense2')
