...
from model import refined_unet3d
...
import numpy as np
from prediction_my import refined_UNet_predict
...
def main():
    ...
    for iFold in range(nFold):
    	...
    	single_model = refined_unet3d(patch_size + [1], patch_size + [1], len(labels))
    	...
    	##### Prediction ###    	
    	predictor = refined_UNet_predict(model, image_size, patch_size, labels)
    	
    	for i in range(len(test_id)):
    	
    		prediction = predictor.__run__(X[test_id[i]], uncertainty[test_id[i]])
    		
    		dice.append(metrics.dice_multi_array(Y[test_id[i]], prediction, labels))
    		save_image(prediction, affines[test_id[i]], '/home/axel/dev/neonatal_brain_segmentation/data/output/refined_U-Net/prediction/allVOI_cross_entropy_loss/' + subject_names[test_id[i]] + '_predicted_segmentation.nii')
    	
    	model.save('/home/axel/dev/neonatal_brain_segmentation/data/output/refined_U-Net/models/model_' + str(iFold) + '.h5')
